class HomeController < ApplicationController
  ...
  def index
    ...
    @medal_img2 = "medal1.PNG"
    @medal_img3 = "medal1.PNG"
  ...
  def day
    ...
    if params.has_key?(:id) then
      if(Time.new.day > params[:id].to_i) then
        redirect_to '/day/25'
      else
        @date = params[:id]
        @day = Information.where(date: params[:id], day:"낮").all
      end
    else
      redirect_to '/day/25'
    end
  ...
  def night
    ...
    if params.has_key?(:id) then
      if(Time.new.day > params[:id].to_i) then
        ...
        @date = params[:id]
        @night = Information.where(date: params[:id], day:"밤").all
      end
    else
      redirect_to '/night/25'
    end
